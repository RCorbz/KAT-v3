# 🏭 Keep America Trucking: Software Factory Directives (Gemini 3.1 Pro Edition)
...
* **Telephony & Messaging:** Twilio Node.js SDK (Voice Webhooks & SMS) and SendGrid (Email).
...
DATABASE_URL="", BETTER_AUTH_SECRET="", BETTER_AUTH_URL="http://localhost:3000", UPLOADTHING_SECRET="", UPLOADTHING_APP_ID="", GOOGLE_CLOUD_PROJECT="", GOOGLE_CLOUD_LOCATION="us-central1", GOOGLE_CLIENT_EMAIL="", GOOGLE_PRIVATE_KEY="", SQUARE_ACCESS_TOKEN="", SQUARE_ENVIRONMENT="sandbox", COMMUNICATION_MODE="mock", TWILIO_ACCOUNT_SID="", TWILIO_AUTH_TOKEN="", DEFAULT_TWILIO_NUMBER="", DR_BEN_CELL_NUMBER="", CRON_SECRET=""
...
├── app/
│   ├── (public)/                 
│   │   ├── page.tsx              # Hero, Ticker Tape, Call Dr. Ben CTA
│   │   ├── locations/[slug]/page.tsx 
│   │   └── layout.tsx            # BottomNavBar
├── admin/                    
│   │   ├── layout.tsx            # STRICT MIDDLEWARE INTERCEPTION
│   │   ├── page.tsx              # Crystal Ball BI Dashboard
│   │   ├── form-builder/page.tsx # CRUD for IntakeQuestions
│   │   ├── campaigns/page.tsx    # Retention Engine CRUD
│   │   ├── operations/page.tsx   # Pricing, Duration, & Schedule CRUD
│   │   └── reputation/page.tsx   # Review Triage & AI Theme Dashboard
├── get-my-card/              
│   │   ├── page.tsx              # Multimodal Intake + Upsell
│   │   └── success/page.tsx      
├── feedback/
│   │   └── [appointmentId]/page.tsx # The Smart Review Gate
├── api/                      
│       ├── cron/retention/route.ts 
│       ├── intake/parse/route.ts 
│       ├── metrics/route.ts      
│       ├── feedback/process/route.ts # AI Theme Extraction
│       └── plivo/webhook/call/route.ts 
├── lib/
│   ├── communication.ts, db.ts, auth.ts, square.ts, vertex.ts                 
└── db/
    ├── schema.ts, index.ts, seed.ts

--------------------------------------------------------------------------------
6. CORE ARCHITECTURAL RULES (ZERO HARDCODING)
1. Dynamic Operations: You are strictly forbidden from hardcoding service prices, appointment durations, or clinic schedules. The frontend MUST derive these values by querying the Drizzle database.
2. Mobile-First UX: Design for touch. Minimum tap target size is 44px. Eliminate unnecessary clicks. The booking flow (Select Time -> Upsell -> Confirm) must be ≤ 3 interactions.
...
• Voice-to-JSON: Audio streamed to /api/intake/parse. Fetch active IntakeQuestions from the DB, feed to Vertex AI, and force a JSON schema output. Auto-check the UI form boxes based on the result.
...
• The Cron Worker (/api/cron/retention): Authenticate via CRON_SECRET. Query due users. Dispatch SMS/Email via `CommunicationService`. Log to RetentionLog. Prevent duplicate sends in the same phase.
...
• Seeding (db/seed.ts): Must be idempotent (upsert). Seed Weatherford TX (Active), SLC UT (Fall 2026), Standard 9-5 Schedule, DOT Physical (130,30m),Tune−Up(100, 15m), 5 Medical Questions, and 3 mock 5-star reviews flagged isFeatured: true.
...
• Dynamic Multimodal AI: Admin adds jsonKey: "hasSleepApnea"; /api/intake/parse MUST instruct Vertex AI to include "hasSleepApnea": boolean in JSON output.
• Single Calendar Upsell: Base service (30m) + upsell (15m) MUST equal a single 45-minute calendar block.
• Campaign Variable Parsing: Cron MUST swap {FirstName} before sending SMS via CommunicationService.
• Square BI Accuracy: If Square AOV is $135, and 10 users are due in 2 weeks with a 50% historical retention rate, 2-week projection MUST output $675.
• AI Theme Extraction: A 2-star review reading "Doctor was 30 mins late" MUST be processed by Vertex AI, assigned a theme like "Wait Time", and saved to the DB without hitting Google.
Segment B: Expected Error Handling (Resilience)
• Telephony Fallback: If Dr. Ben rejects the call, Twilio webhook MUST catch 'busy' status and dispatch the auto-SMS to the caller.
• Voice Parsing Fallback: If Vertex AI returns { "confidence": "low" }, display a toast ("Could not hear clearly") and leave boxes blank. Do not crash.
• Square API Outage: If Square returns 500, /admin MUST fallback to calculating AOV using Drizzle Service prices.
• Double-Booking Rejection: Simultaneous booking attempts for the same slot MUST result in 409 Conflict for the second user.
• Unauthenticated Cron Access: /api/cron/retention without CRON_SECRET MUST return 401 Unauthorized.
Segment C: Low-Friction UX & Navigation
• The 3-Click Checkout: From Time Select -> Upsell -> Confirm MUST be ≤ 3 interactions.
• Instant Visual Confirmation: Successful booking MUST instantly route to /get-my-card/success with a green checkmark and "Return to Home" CTA.
• Review Gate UX: A 5-star click on /feedback MUST bypass text entry, show confetti, and redirect immediately to the external Google Review URL.
• Dead-End Avoidance: Unauthenticated users hitting /admin MUST route to /sign-in. Upon login, BetterAuth MUST redirect them to their intended /admin destination.
• Ticker Tape UX: The 5-star review ticker MUST auto-play using CSS keyframes/Framer Motion to continuously animate horizontally upon page load without user interaction.
• Franchise UX: Navigating to SLC location MUST display "Opening Fall 2026", completely disable the booking calendar, and hide the intake form.
Segment D: Database Integrity
• Idempotent Seeding: Running the seed script 3 times MUST NOT create duplicate clinics or services.
• Retention Math: nextDueDate MUST correctly calculate exactly 2 years from scheduledFor.
• Cascade Deletion: Deleting a User MUST cascade to delete associated Appointment, Review, and RetentionLog records.
...
* **SWITCH TO GEMINI 3.1 PRO (High Thinking) FOR:**
    * Initial project scaffolding and complex Drizzle schema migrations.
    * Architecting the Twilio Voice Webhook logic and multi-step redirection.
