...
├── lib/
│   ├── prisma.ts, auth.ts, square.ts, plivo.ts, vertex.ts                 
└── prisma/
...
• Seeding (prisma/seed.ts): Must be idempotent (upsert). Seed Weatherford TX (Active), SLC UT (Fall 2026), Standard 9-5 Schedule, DOT Physical (130,30m),Tune−Up(100, 15m), 5 Medical Questions, and 3 mock 5-star reviews flagged isFeatured: true.
...
• Campaign Variable Parsing: Cron MUST swap {FirstName} before sending Plivo SMS.
...
• Telephony Fallback: If Dr. Ben rejects the call, Plivo webhook MUST catch 'busy' status and dispatch the auto-SMS to the caller.
...
• Square API Outage: If Square returns 500, /admin MUST fallback to calculating AOV using Prisma Service prices.
...
• Idempotent Seeding: npx prisma db seed run 3 times MUST NOT create duplicate clinics or services.
